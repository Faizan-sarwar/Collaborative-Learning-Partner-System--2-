...
// Emails force-promoted to super-admin regardless of their stored role.
// Keep this list in sync with the matching list in Signup.jsx.
const ADMIN_EMAILS = ["faizan@admin.com"];
...
const AdminManagement = () => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const res = await fetch(
        `http://${window.location.hostname}:5000/api/auth/admin/admins`,
        {
      ...
      if (data.success) {
        ...
        const filteredList = processedAdmins.filter(
          (user) => user.role && allowedRoles.includes(user.role.toLowerCase()),
        );
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/auth/admin/admins/${_id}`,
        {
          method: "DELETE",
  ...
  const attemptDelete = (admin) => {
    if (admin.role === "super-admin")
      return alert("Action Denied: Cannot delete the Super Admin.");
    if (currentUser && currentUser._id === admin._id)
      return alert("Action Denied: Cannot delete your own account.");
              ...
              admins.map((admin) => {
                ...
                return (
                  <tr key={admin._id}>
                      ...
                      <div className={styles.adminInfo}>
                        ...
                        <div className={styles.details}>
                          <span className={styles.name}>
                            {admin.fullName}{" "}
                            {isSelf && (
                              <strong className={styles.selfTag}>(You)</strong>
                            )}
                          </span>
                    ...
                    <td>
                      <span
                        className={`${styles.roleBadge} ${styles[admin.role] || styles.admin}`}
                      >
                    ...
                    <td>
                      <span
                        className={`${styles.status} ${admin.approved ? styles.active : styles.inactive}`}
                      >
                    ...
                    <td>
                      <div className={styles.actions}>
                        <button
                          ...
                          disabled={isDisabled}
                          title={
                            isDisabled ? "Cannot edit this user" : "Edit Admin"
                          }
                        >
                          <Edit size={16} />
                        </button>
          ...
          >
            ...
            >
              <div className={styles.modalHeader}>
                <h3>
                  {modalMode === "add" ? "Add New Admin" : "Edit Admin Details"}
                </h3>
                <button
          ...
          >
            ...
            >
              ...
              <p>
                ...
                <strong>{showDeleteModal.fullName}</strong>? This action cannot
                be undone.
              </p>
