...
const UserProfile = () => {
    ...
    if (error || !user) return <><div className={styles.container}><h3>{error || "User not found"}</h3></div></>;
            ...
            <div className={styles.container} style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2.5fr', gap: '24px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        {/* Reliability Card */}
                        <div style={{ backgroundColor: 'var(--bg-secondary, #1e293b)', borderRadius: '12px', padding: '20px', border: '1px solid var(--border-color, #334155)' }}>
                            <h3 style={{ margin: '0 0 15px 0', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary, #f8fafc)' }}>
                                <Shield size={18} color={trustTier.color} /> Trust Score
                            </h3>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                <div style={{ position: 'relative', width: '60px', height: '60px' }}>
                                    <svg viewBox="0 0 36 36" style={{ width: '100%', height: '100%' }}>
                                        <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="var(--border-color, #334155)" strokeWidth="4" />
                                    ...
                                    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.9rem', color: 'var(--text-primary, #f8fafc)' }}>
                                        {user.reliability || 0}%
                                    </div>
                                </div>
                                <div>
                                    <div style={{ color: trustTier.color, fontWeight: '600', fontSize: '1rem' }}>{trustTier.label}</div>
                                    <div style={{ color: 'var(--text-muted, #94a3b8)', fontSize: '0.8rem', marginTop: '2px' }}>Based on quiz & activity</div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* About Card */}
                        <div style={{ backgroundColor: 'var(--bg-secondary, #1e293b)', borderRadius: '12px', padding: '20px', border: '1px solid var(--border-color, #334155)' }}>
                            <h3 style={{ margin: '0 0 15px 0', fontSize: '1.1rem', color: 'var(--text-primary, #f8fafc)' }}>About</h3>
                            <p style={{ color: 'var(--text-secondary, #cbd5e1)', fontSize: '0.95rem', lineHeight: '1.5', margin: 0 }}>
                    ...
                    <div style={{ backgroundColor: 'var(--bg-secondary, #1e293b)', borderRadius: '12px', border: '1px solid var(--border-color, #334155)', overflow: 'hidden' }}>

                        {/* Custom Tabs */}
                        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color, #334155)', backgroundColor: 'var(--bg-primary, #0f172a)' }}>
                            {[
                                ...
                                <button
                                    ...
                                    style={{ flex: 1, padding: '15px', background: activeTab === tab.id ? 'var(--bg-secondary, #1e293b)' : 'transparent', border: 'none', borderBottom: activeTab === tab.id ? '2px solid #3b82f6' : '2px solid transparent', color: activeTab === tab.id ? '#3b82f6' : 'var(--text-muted, #94a3b8)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer', fontWeight: '500', transition: 'all 0.2s' }}
                                >
                                    {tab.icon} {tab.label}
                                </button>
                            ))}
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {activeTab === 'overview' && (
                                    <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                        ...
                                        {/* Quick Stats Grid */}
                                        <h3 style={{ color: 'var(--text-primary, #f8fafc)', fontSize: '1.1rem', margin: '0 0 15px 0' }}>Platform Stats</h3>
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', marginBottom: '30px' }}>
                                            <div style={{ backgroundColor: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.2)', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                                                ...
                                                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-primary, #f8fafc)' }}>Level {user.level || 1}</div>
                                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted, #94a3b8)' }}>{user.xp || 0} XP</div>
                                            </div>
                                            ...
                                            <div style={{ backgroundColor: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.2)', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                                                ...
                                                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-primary, #f8fafc)' }}>{user.streak || 0}</div>
                                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted, #94a3b8)' }}>Day Streak</div>
                                            </div>
                                        </div>

                                        {/* Skills Section */}
                                        <h3 style={{ color: 'var(--text-primary, #f8fafc)', fontSize: '1.1rem', margin: '0 0 15px 0' }}>Academic Profile</h3>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                            <div style={{ backgroundColor: 'var(--bg-primary, #0f172a)', padding: '15px', borderRadius: '10px' }}>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                                    {user.academicStrengths?.length > 0
                                                        ...
                                                        : <span style={{ color: 'var(--text-muted, #64748b)', fontSize: '0.85rem' }}>Not specified</span>
                                                    }
                                                </div>
                                            </div>
                                            <div style={{ backgroundColor: 'var(--bg-primary, #0f172a)', padding: '15px', borderRadius: '10px' }}>
                                ...
                                {activeTab === 'academic' && (
                                    <motion.div key="academic" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', backgroundColor: 'var(--bg-primary, #0f172a)', borderRadius: '8px' }}>
                                                <span style={{ color: 'var(--text-muted, #94a3b8)' }}>Department</span>
                                                <span style={{ color: 'var(--text-primary, #f8fafc)', fontWeight: '500' }}>{user.department || 'Not specified'}</span>
                                            </div>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', backgroundColor: 'var(--bg-primary, #0f172a)', borderRadius: '8px' }}>
                                                <span style={{ color: 'var(--text-muted, #94a3b8)' }}>Current Semester</span>
                                                <span style={{ color: 'var(--text-primary, #f8fafc)', fontWeight: '500' }}>Semester {user.semester || 'N/A'}</span>
                                            </div>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', backgroundColor: 'var(--bg-primary, #0f172a)', borderRadius: '8px' }}>
                                                <span style={{ color: 'var(--text-muted, #94a3b8)' }}>Preferred Study Style</span>
                                                <span style={{ color: 'var(--text-primary, #f8fafc)', fontWeight: '500' }}>{user.studyStyle || 'Individual Study'}</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                )}
                                ...
                                {activeTab === 'schedule' && (
                                    <motion.div key="schedule" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                        <div style={{ backgroundColor: 'var(--bg-primary, #0f172a)', padding: '20px', borderRadius: '10px' }}>
                                            <h3 style={{ color: 'var(--text-primary, #f8fafc)', fontSize: '1.1rem', margin: '0 0 15px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <Calendar size={18} color="#3b82f6" /> Typical Availability
                                            </h3>
                                            <p style={{ color: 'var(--text-secondary, #cbd5e1)', lineHeight: '1.6', margin: 0, padding: '15px', backgroundColor: 'rgba(59, 130, 246, 0.05)', borderLeft: '3px solid #3b82f6', borderRadius: '0 8px 8px 0' }}>
