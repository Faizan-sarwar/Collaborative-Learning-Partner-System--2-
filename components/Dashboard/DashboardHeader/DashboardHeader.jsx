...
const DashboardHeader = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // 1. Tell Server to set status to Offline
  ...
  return (
    <header className={styles.header}>
      ...
      <div className={styles.actions}>
            ...
            {showNotifications && (
              <motion.div className={styles.notificationDropdown} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                <div className={styles.notifList}>
                  {notifications.length === 0 ? <div className={styles.emptyNotif}><p>No new notifications</p></div> : 
                    notifications.map((notif) => (
                      ...
                      >
                        <div className={styles.notifIcon}>
                            {notif.sender ? (
                                <img src={getSenderAvatar(notif.sender)} alt="sender" className={styles.notifAvatarImg} style={{width:'32px', height:'32px', borderRadius:'50%', objectFit:'cover'}} onError={(e) => {e.target.style.display='none';}} />
        ...
        {/* <Link to='/study-room' className={styles.studyRooms}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>Study Rooms</Link> */}
        <Link to='/refer' className={styles.referBtn}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>Refer</Link>
