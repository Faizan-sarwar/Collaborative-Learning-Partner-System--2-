...
const DashboardSidebar = () => {
  ...
  // 🟢 NEW: State to hold notification badges
  const [badges, setBadges] = useState({});

  // 🟢 NEW: Calculate where to put the Red Dots
  ...
  // 🟢 NEW: Listen to global events to update dots instantly
  useEffect(() => {
    calculateBadges(); // Run on mount
    window.addEventListener('userUpdated', calculateBadges);
    window.addEventListener('notificationAdded', calculateBadges);
    
    return () => {
      window.removeEventListener('userUpdated', calculateBadges);
      window.removeEventListener('notificationAdded', calculateBadges);
    };
  }, []);
  ...
  const renderIcon = (iconName) => {
    const icons = {
      ...
      match: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /><line x1="20" y1="8" x2="20" y2="14" /><line x1="23" y1="11" x2="17" y2="11" /></svg>),
      ...
      pending: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>),
    ...
    <aside className={styles.sidebar}>
      <div className={styles.header}>
        <div className={styles.logo}>
          ...
          <span className={styles.logoText}>{loadingSettings ? 'Loading...' : settings.platformName}</span>
        </div>
      </div>
      ...
      <nav className={styles.nav}>
        {navItems.map((item) => (
          ...
          >
            {/* 🟢 NEW: Conditionally render the Red Dot! */}
