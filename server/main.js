...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (!uid) return;
    console.log(
      `🔌 Socket ${socket.id} disconnected: ${reason} (uid=${uid}) — cleanup in 1.5s`,
    );
    setTimeout(() => {
...
const MONGO_URI =
  process.env.MONGO_URI || "mongodb://127.0.0.1:27017/studybuddy";
...
app.post("/studygroup", async (req, res) => {
  try {
    const { name, subjects, description, meetingTime, membership, creatorId } =
      req.body;
    ...
    if (!creatorId || !mongoose.Types.ObjectId.isValid(creatorId))
      return res
        .status(400)
        .json({ success: false, message: "Valid creator ID is required" });
    ...
    if (err.name === "ValidationError") {
      ...
      return res
        .status(400)
        .json({ success: false, message: errors[0].message, errors });
    }
    res
      .status(500)
...
cron.schedule("0 0 * * *", async () => {
  ...
  try {
    ...
    if (settings.dataRetention) {
      ...
      console.log(
        `🧹 [CRON] Data Retention: Deleted ${deletedLogs.deletedCount} old activity logs.`,
      );
    }
    ...
    if (settings.autoBackup === "weekly" && dayOfWeek === 0)
      shouldBackup = true;
    if (settings.autoBackup === "monthly" && dateOfMonth === 1)
      shouldBackup = true;

    if (shouldBackup) {
      console.log(
        `💾 [CRON] Auto Backup Triggered (${settings.autoBackup} schedule).`,
      );
    }
  } catch (err) {
