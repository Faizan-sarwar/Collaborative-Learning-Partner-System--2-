...
const generateToken = (id) =>
  jwt.sign({ id }, process.env.JWT_SECRET || "test_key", {
...
const validateSignupData = (data) => {
  ...
  if (!data.fullName?.trim()) addErr("fullName", "Full name is required");
    ...
    addErr("fullName", "Name must be at least 2 characters");
  else if (data.fullName.trim().length > 100)
    addErr("fullName", "Name must be less than 100 characters");
  ...
  if (!data.password) addErr("password", "Password is required");
    ...
    if (issues.length)
      addErr("password", `Password must contain ${issues.join(", ")}`);
  }
  ...
  if (
    ...
    ![
      ...
      "Civil",
      "Electrical",
    ].includes(data.department)
  )
    addErr("department", "Invalid department");
  if (
    !data.semester ||
  ...
  if (data.availability?.length > 500)
    addErr("availability", "Availability must be less than 500 characters");
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ success: false, message: "No such email exists" });
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (!storedData)
      return res
        .status(400)
        .json({ success: false, message: "OTP expired or was not requested." });
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (settings && settings.maintenanceMode) {
      if (user.role !== "admin" && user.role !== "super-admin") {
        return res.status(503).json({
          success: false,
          message:
            "The platform is currently under maintenance. Only administrators can log in right now.",
        });
      }
    }
    ...
    if (today.toDateString() !== previousLoginDate.toDateString()) {
      ...
      user.streak =
        yesterday.toDateString() === previousLoginDate.toDateString()
          ? (user.streak || 0) + 1
          : 1;
    ...
    res.json({
      success: true,
      token: jwt.sign({ id: user._id }, process.env.JWT_SECRET || "test_key", {
...
router.get("/me", async (req, res) => {
  try {
    ...
    const user = await User.findById(
      jwt.verify(token, process.env.JWT_SECRET || "test_key").id,
    );
    if (!user)
      return res
        .status(404)
...
router.get("/admin/dashboard", async (req, res) => {
  try {
    res.json({
      ...
      pendingApprovals: await User.countDocuments({
        approved: false,
        role: "student",
      }),
...
router.get("/admin/analytics", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        registrations: registrationStats.map((s) => ({
        ...
        status: [
          {
            name: "Active",
            value: Math.max(activeCount - inactiveCount, 0),
            color: "#10B981",
          },
...
router.get("/admin/students", async (req, res) => {
  try {
    const students = await User.find({
      ...
      .select(
        "fullName email rollNumber department approved lastLogin createdAt semester isOnline",
      )
      .sort({ createdAt: -1 });
    const now = new Date();
    res.json({
      ...
      students: students.map((s) => ({
        ...
        status:
          s.approved === false
            ? "blocked"
            : s.isOnline && now - new Date(s.lastLogin) < 15 * 60 * 1000
              ? "active"
              : "logged out",
...
router.put("/admin/students/:id", async (req, res) => {
  try {
    ...
    else if (["active", "logged out"].includes(status)) user.approved = true;
    await user.save();
...
router.get("/admin/admins", async (req, res) => {
  try {
    res.json({
      success: true,
      admins: await User.find({ role: { $ne: "student" } })
...
router.post("/admin/create-admin", async (req, res) => {
  try {
    ...
    if (role === "super-admin" || !["admin", "moderator"].includes(role))
      return res
        .status(403)
        .json({ success: false, message: "Invalid or restricted role." });
...
router.get("/admin/settings", async (req, res) => {
  try {
    res.json({
      success: true,
      settings: (await Settings.findOne()) || (await Settings.create({})),
    });
  } catch (err) {
    res.status(500).json({ success: false, message: "Server error" });
  }
  ...
  try {
    res.json({
      ...
      notifications: await Notification.find({
        type: { $in: ["system", "registration", "admin"] },
      })
        .sort({ createdAt: -1 })
        .limit(20),
    });
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        activeGroups: (await StudyGroup.find({ members: user._id })).length,
        level: user.level || 1,
        xp: user.xp || 0,
      },
    });
...
router.put("/update-stats", async (req, res) => {
  try {
    ...
    if (tasksCompleted !== undefined)
      updateFields.tasksCompleted = tasksCompleted;
    if (streakData) {
...
router.post("/submit-quiz", async (req, res) => {
  try {
    ...
    const baseScore = 40;
    const earnedScore =
      totalQuestions > 0 ? Math.round((score / totalQuestions) * 58) : 0;
...
router.get("/matches/:userId", async (req, res) => {
  try {
    ...
    const diffs = Array.isArray(currentUser.subjectsOfDifficulty)
      ? currentUser.subjectsOfDifficulty
      : [];
    ...
    const connections = Array.isArray(currentUser.connections)
      ? currentUser.connections.map(safeToString)
      : [];
    const sentReqs = Array.isArray(currentUser.sentRequests)
      ? currentUser.sentRequests.map(safeToString)
      : [];
    const receivedReqs = Array.isArray(currentUser.receivedRequests)
      ? currentUser.receivedRequests.map(safeToString)
      : [];

    const matches = candidates.map((u) => {
      const theirStrengthsRaw = Array.isArray(u.academicStrengths)
        ? u.academicStrengths
        : [];
        ...
        (
          await User.findById(
            jwt.verify(token, process.env.JWT_SECRET || "your_jwt_secret_key")
              .id,
          ).populate({ path: "connections", select: "-picture.data" })
        ).connections || [],
    });
  } catch (err) {
    res.status(500).json({ success: false, message: "Server error" });
...
router.get("/requests/:type", async (req, res) => {
  try {
    ...
    if (!token) return res.status(401).json({ message: "Unauthorized" });
    const user = await User.findById(
      jwt.verify(token, process.env.JWT_SECRET || "your_jwt_secret_key").id,
    ).populate({
      path: req.params.type === "sent" ? "sentRequests" : "receivedRequests",
      select: "-picture.data",
    });
    res.json({
      success: true,
      requests:
        req.params.type === "sent"
          ? user?.sentRequests
...
router.post("/connect/:targetId", async (req, res) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    if (!token) return res.status(401).json({ message: "Unauthorized" });
    const senderId = jwt.verify(token, process.env.JWT_SECRET || "test_key").id,
    ...
    await Notification.create({
      ...
      message: `${senderUser.fullName} wants to connect.`,
  ...
  try {
    ...
    if (action === "accept") {
      ...
      await Notification.create({
        ...
        message: `${receiver.fullName} accepted your request! +75 XP`,
        link: "/messages",
        unread: true,
      });
      await Notification.create({
        ...
        message: `You are now connected with ${sender.fullName}. +75 XP`,
        link: "/messages",
        unread: true,
      });
...
router.post("/requests/:id/accept", async (req, res) => {
  try {
    ...
    if (typeof receiver.adjustReliability === "function")
      await receiver.adjustReliability(0.5);
    if (typeof sender.adjustReliability === "function")
      await sender.adjustReliability(0.5);
    ...
    pushLiveNotification(req, senderId, {
      type: "connection",
      title: "Request Accepted 🎉",
      message: `${receiver.fullName} accepted your connection request! +75 XP`,
    });
...
router.post("/award-xp", async (req, res) => {
  try {
    ...
    const qualifiedLevelObj = [...MILESTONES]
      .reverse()
      .find((m) => user.xp >= m.xp && (user.studyHours || 0) >= m.hours);
...
router.post("/ai/chat-topics", async (req, res) => {
  try {
    ...
    if (!process.env.GROQ_API_KEY) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "API Credentials missing" });
    }
    ...
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You output strict JSON only. No markdown formatting.",
        },
...
router.get("/admin/export-data", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-disposition",
      `attachment; filename=platform_backup_${Date.now()}.json`,
    );
...
router.post("/admin/clear-cache", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Server memory and caches cleared successfully!",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: "Server error" });
  }
...
router.post("/contact", async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      text:
        `You received a new message from your platform contact form:\n\n` +
        `Name: ${name}\n` +
        `Email: ${email}\n\n` +
        `Message:\n${message}`,
      html: `
